...
def dataset_parser(ai_message_content):
    ...
    bracket_start = ai_message_content.find(start_mark) + len(start_mark) - 1
    bracket_end = ai_message_content.find(end_mark)
    
    json_body = ai_message_content[bracket_start : bracket_end + 1]

    # Validate json body
    json_body = re.sub(r"\s", "", json_body)
    json_body = re.sub(r",}", "}", json_body)
...
def main(debug=False):
    ...
    raw_content_data = []
    with open(os.path.join(data_dir, 'raw_data.txt'), 'r', encoding='utf-8') as file:
        content = file.read()
    ...
    filename = f"zhouyi_dataset_{timestamp}.csv"
